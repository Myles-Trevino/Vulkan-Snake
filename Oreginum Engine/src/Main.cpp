...
int WinMain(HINSTANCE instance, HINSTANCE previous_instance, LPSTR arguments, int show)
{
	...
	Vulkan vulkan{window, "Oreginum Engine Vulkan Test", {0, 1, 0},
		...
		vertex_attribute_descriptions, vertices, indices, &uniforms, sizeof(uniforms), true};
	...
	while(window.update())
	{
		...
		uniforms.mvp[1][1] *= -1;
		uniforms.mvp *= glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f))*glm::rotate(glm::mat4(), time_elapsed*glm::radians(90.0f),

...
public:
	Vulkan(const Window& window, const std::string& program_title,
		...
		const glm::ivec3& engine_version, const glm::ivec3& vulkan_version, bool debug = false);
...
private:
	...
	void check_extension_support(const std::vector<const char *>& extensions);
	void check_layer_support(const std::vector<const char *>& layers);

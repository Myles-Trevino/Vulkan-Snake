...
namespace
{
	...
	void destroy_debug_callback(VkInstance instance, VkDebugReportCallbackEXT callback,
	...
	void create_surface()
	{
		...
		if(vkCreateWin32SurfaceKHR(instance, &surface_information, nullptr, &surface) != VK_SUCCESS)
			Oreginum::Core::error("Could not create Vulkan surface.");
	}
	...
	void select_gpu()
	{
		...
		for(const auto& g : gpus)
		{
			...
			if(!(swapchain_formats.size() == 1 &&
				swapchain_formats[0].format == VK_FORMAT_UNDEFINED))
			{
	...
	void create_swapchain()
	{
		...
		if(vkCreateSwapchainKHR(device, &swapchain_information, nullptr, &new_swapchain) !=
	...
	void create_render_pass()
	{
		...
		if(vkCreateRenderPass(device, &pass_information, nullptr, &render_pass) != VK_SUCCESS)
			Oreginum::Core::error("Could not create Vulkan render pass.");
	}
	...
	Vulkan_Deleter<VkShaderModule> create_shader(const std::string& shader)
	{
		...
		if(vkCreateShaderModule(device, &shader_information, nullptr, &module) !=
			VK_SUCCESS) Oreginum::Core::error("Could not create shader \""+shader+"\".");
	...
	void create_descriptor_set_layout()
	{
		...
		if(vkCreateDescriptorSetLayout(device, &layout_information, nullptr,
			...
			Oreginum::Core::error("Could not create Vulkan descriptor set layout.");
	}
	...
	void create_graphics_pipeline()
	{
		...
		depth_stencil_information.sType =
		...
		if(vkCreatePipelineLayout(device, &pipeline_layout_information,
	...
	void create_framebuffers()
	{
		...
		for(size_t i{}; i < swapchain_image_views.size(); ++i)
		{
			...
			if(vkCreateFramebuffer(device, &framebuffer_information, nullptr, &framebuffers[i]) !=
	...
	uint32_t find_memory(uint32_t type, VkMemoryPropertyFlags properties)
	{
		...
		Oreginum::Core::error("Your GPU ran out of memory.");
	}
		...
		Vulkan_Deleter<VkImage>& texture, Vulkan_Deleter<VkDeviceMemory>& texture_memory)
	{
		...
		if(vkAllocateMemory(device, &allocation_information, nullptr, texture_memory.replace())
			!= VK_SUCCESS) Oreginum::Core::error("Could not allocate Vulkan image memory.");
	...
	VkCommandBuffer begin_single_time_commands()
	{
		...
		command_buffer_allocation_information.sType =
	...
	void transition_image_layout(VkImage texture, VkFormat format,
		VkImageLayout old_layout, VkImageLayout new_layout)
	{
		...
		else Oreginum::Core::error("Could not complete a Vulkan texture layout transition.");
	...
	void create_texture()
	{
		...
		if(!data) Oreginum::Core::error("Could not load the image \""+model->get_texture()+"\".");
		...
		transition_image_layout(texture, VK_FORMAT_R8G8B8A8_UNORM,
			VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
	}

	void create_texture_view()
	{
		create_image_view(texture, VK_FORMAT_R8G8B8A8_UNORM,
			VK_IMAGE_ASPECT_COLOR_BIT, texture_view);
	}
	...
	void create_texture_sampler()
	{
		...
		if(vkCreateSampler(device, &sampler_information, nullptr, texture_sampler.replace())
			!= VK_SUCCESS) Oreginum::Core::error("Could not create a Vulkan texture sampler.");
	}
		...
		Vulkan_Deleter<VkDeviceMemory>& buffer_memory)
	{
		...
		if(vkCreateBuffer(device, &buffer_information, nullptr, buffer.replace()) != VK_SUCCESS)
			Oreginum::Core::error("Could not a create Vulkan buffer.");
		...
		if(vkAllocateMemory(device, &allocation_information, nullptr, buffer_memory.replace()) !=
			VK_SUCCESS) Oreginum::Core::error("Could not allocate Vulkan buffer memory.");

		vkBindBufferMemory(device, buffer, buffer_memory, 0);
	}

	void copy_buffer(VkBuffer source_buffer,
	...
	void create_command_buffers()
	{
		...
		buffer_allocation_information.commandBufferCount =
			static_cast<uint32_t>(command_buffers.size());
		if(vkAllocateCommandBuffers(device, &buffer_allocation_information, command_buffers.data())
			!= VK_SUCCESS) Oreginum::Core::error("Could not create Vulkan command buffers.");
		...
		for(size_t i{}; i < command_buffers.size(); i++)
		{
			...
			clear_colors[0] = {0.047f, 0.075f, 0.098f, 1};
			clear_colors[1] = {1, 0};
			...
			if(vkEndCommandBuffer(command_buffers[i]) != VK_SUCCESS)
				Oreginum::Core::error("Could not record commands to a Vulkan command buffer.");
		}
	}
...
void Oreginum::Vulkan::initialize(const Model *model, const void *uniform_buffer_object,
...
void Oreginum::Vulkan::destroy() { vkDeviceWaitIdle(device); }
